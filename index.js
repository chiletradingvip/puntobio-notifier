...
async function enviarCorreo({ pedidoId, nombre, telefono, email, direccion, items, total, envio, metodoPago, tipoEntrega }) {
  try {
    ...
    const envioTexto = tipoEntrega === "retiro" ? "Retiro gratis" : (envio || 0) > 0 ? `$${(envio||0).toLocaleString("es-CL")}` : "GRATIS";
    const metodoBadge = metodoPago === "Tarjeta (Mercado Pago)" ? "💳" : metodoPago === "Transferencia bancaria" ? "📲" : "💵";
    const totalReal = (items || []).reduce((s, i) => s + i.precioPorLitro * i.litros, 0) + (envio || 0);
  ...
  <div style="max-width:480px;margin:20px auto;background:#fff;border-radius:14px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.1)">
    ...
    <div style="padding:16px">
      ...
      <table width="100%" style="border-collapse:collapse;background:#f8fafc;border-radius:8px;overflow:hidden;margin-bottom:14px">
        <thead><tr style="background:#1a6dbf">
        ...
        <tbody>${itemsRows}
          <tr style="background:#e8f4fd">
            <td style="padding:8px 10px;font-size:12px;color:#555">${tipoEntrega === "retiro" ? "🏪 Retiro en tienda" : (envio||0) === 0 ? "🚚 Envío gratis" : "📦 Envío"}</td>
          ...
          <tr style="background:#0a3a6e">
            ...
            <td style="padding:12px 10px;text-align:right;color:#00d4ff;font-weight:900;font-size:16px;white-space:nowrap">$${totalReal.toLocaleString("es-CL")}</td>
          </tr>
        </tbody>
      </table>
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [NOTIFY_EMAIL],
        subject: `🛒 Pedido ${pedidoId} — $${totalReal.toLocaleString("es-CL")} — ${metodoPago}`,
        html
      })
    });
...
app.get("/", (req, res) => res.json({ ok: true, service: "PuntoBio Backend v4" }));

// Efectivo y Transferencia → confirmar y notificar inmediatamente
app.post("/pagar", async (req, res) => {
  ...
  // Efectivo o Transferencia → notificar inmediatamente
  if (metodoPago === "Efectivo presencial" || metodoPago === "Transferencia bancaria") {
    enviarCorreo({ pedidoId, nombre, telefono, email, direccion, items, total, envio: envio || 0, metodoPago, tipoEntrega }).catch(console.error);
    return res.json({ ok: true, status: "confirmed" });
  }

  // Tarjeta → crear preferencia MP
  try {
    const response = await fetch("https://api.mercadopago.com/checkout/preferences", {
      ...
      body: JSON.stringify({
        items: items.map(i => ({ id: String(i.id), title: `${i.nombre} ${i.litros}L`, quantity: 1, unit_price: Math.round(Number(i.precioPorLitro) * Number(i.litros)), currency_id: "CLP" }))
          .concat((envio||0) > 0 ? [{ id: "envio", title: "Costo de envio", quantity: 1, unit_price: Math.round(Number(envio)), currency_id: "CLP" }] : []),
        payer: { name: nombre, email: email || NOTIFY_EMAIL, phone: { number: telefono } },
    ...
    console.log("MP respuesta init_point:", !!data.init_point);
    if (data.init_point) {
      return res.json({ ok: true, url: data.init_point });
    }
    console.error("MP sin init_point:", JSON.stringify(data));
...
app.listen(PORT, () => console.log(`PuntoBio Backend v4 corriendo en puerto ${PORT}`));
